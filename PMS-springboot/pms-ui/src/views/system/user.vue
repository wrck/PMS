...
import { getUserList, addUser, updateUser, deleteUser, resetPassword } from '@/api'
...
const queryForm = reactive({ pageNum: 1, pageSize: 10, username: '', realname: '' })
const formData = reactive({ id: null, username: '', realname: '', email: '', phone: '', deptId: null, status: 1 })
...
const resetQuery = () => { Object.assign(queryForm, { username: '', realname: '' }); handleQuery() }

const handleAdd = () => {
  dialogTitle.value = '新增用户'
  Object.assign(formData, { id: null, username: '', realname: '', email: '', phone: '', deptId: null, status: 1 })
  dialogVisible.value = true
}
const handleEdit = (row) => { dialogTitle.value = '编辑用户'; Object.assign(formData, row); dialogVisible.value = true }

const handleSubmit = async () => {
  if (formData.id) { await updateUser(formData); ElMessage.success('更新成功') }
  else { await addUser(formData); ElMessage.success('创建成功') }
...
const handleResetPwd = (row) => {
  ElMessageBox.confirm(`确认重置 ${row.realname} 的密码？`, '提示', { type: 'warning' }).then(async () => {
