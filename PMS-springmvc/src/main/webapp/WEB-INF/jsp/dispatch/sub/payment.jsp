...
<body>
	<div style="text-align: left;">
        <s:form enctype="multipart/form-data" id="dispatchPaymentForm" action="module/dispatch_savePayment.action">
            <table id="dispatchInfoTable" class="table table-bordered table-hover table-striped ">
                <tr style="display:none"><td><s:hidden name="dispatch.id"/></td></tr>
                <tr>
                    <td><s:text name="pm.dispatch.dispatchNo"></s:text>:</td>
                    <td class="col-sm-4"><s:textfield name="dispatch.dispatchNo" cssClass="form-control" placeholder='请输入转包合同号'/></td>
                    <td><s:text name="pm.dispatch.accrued"></s:text>:</td>
                    <td><s:radio name="dispatch.isAccrued" list="#{true:'是',false:'否'}"/></td>
                    <td><s:text name="pm.dispatch.invoiced"></s:text>:</td>
                    <td><s:radio name="dispatch.isInvoiced" list="#{true:'是',false:'否'}"/></td>
                    <td><s:text name="pm.dispatch.deliver"></s:text>:</td>
                    <td>
                        <span id="finalContracts">
                            <s:text name="pm.dispatch.deliver.contract.final"/>
            ...
            <display:table style="text-align: left;"
	            name="dispatchPaymentList" export="false" id="dispatchPaymentTable"
	            size="${dispatchPaymentList.size()}" sort="external" requestURI="module/sub/querySubcontractPayment.action"
                ...
                <display:column title="序号" headerClass="text-center" class="text-center rowNum">
                    ${dispatchPaymentTable_rowNum}
                </display:column>
                <display:column property="id" headerClass="hidden" class="text-input id hidden" title="hidden" media="html"></display:column>
                <display:column property="dispatchId" headerClass="hidden" class="text-input dispatchId hidden ignore" title="hidden" media="html"></display:column>
                <display:column property="ratio" class="text-input ratio" titleKey="pm.dispatch.ratio"></display:column>
                <display:column property="amount" class="text-input amount" titleKey="pm.dispatch.amount"></display:column>
                <display:column property="confirmTime" class="text-input confirmTime" titleKey="pm.dispatch.confirmTime" format="{0,date,yyyy-MM-dd}"></display:column>
                <display:column property="paymentTime" class="text-input paymentTime" titleKey="pm.dispatch.paymentTime" format="{0,date,yyyy-MM-dd}"></display:column>
                <display:column property="remark" class="text-input remark" titleKey="pm.dispatch.remark"></display:column>
                <display:column title="操作" media="html">
                    <span class="glyphicon glyphicon-plus text-success"></span>
                    <span class="glyphicon glyphicon-minus text-danger"></span>
                </display:column>
	            <display:footer>
	                <s:if test="%{dispatchPaymentList.size() == 0}">
	                    <tr class="empty-template hidden">
	                        <td class="text-center rowNum">1</td>
	                        <td class="text-input dispatchId hidden ignore"><input name="dispatchPaymentList[0].dispatchId" value='<s:property value="dispatch.id"/>' class="form-control"></td>
	                        <td class="text-input ratio"><input name="dispatchPaymentList[0].ratio" class="form-control"></td>
	                        <td class="text-input amount"><input name="dispatchPaymentList[0].amount" class="form-control"></td>
                            <td class="text-input confirmTime"><input name="dispatchPaymentList[0].confirmTime" class="form-control"></td>
	                        <td class="text-input paymentTime"><input name="dispatchPaymentList[0].paymentTime" class="form-control"></td>
	                        <td class="text-input remark"><input name="dispatchPaymentList[0].remark" class="form-control"></td>
	                        <td>
	                            <span class="glyphicon glyphicon-plus text-success"></span>
	                ...
	                <tr id="sumTr" class='text-danger' style='font-weight: 800;'>
	                    ...
	                    <td class="mergeTdBorder">
	                        <s:if test="%{(user.isHasRole(10) || user.isHasRole(13)) && dispatch.state != 100}">
	        ...
	        <div id="emApproveDiv" style="margin-bottom:20px;">
                ...
                <s:if test="%{workflowCommonParam.outcome == 'applyPaymentTask'}">
	                ...
	                <script type="text/javascript">
	                    $(function(){
	                    	$("#applyPaymentBtn").appendTo($("#dispatchPaymentTable tfoot #delIds"));
	                    });
	                </script>
                </s:if>
        ...
        <script type="text/javascript">
	        $(function(){
	        	...
	        	function calcRatio() {
	        		var sumRatio = 0;
	        		var isValid = true;
	                $("#dispatchPaymentTable tbody .ratio").each(function(){
	        	...
	        	calcRatio();
	        	//$("#dispatchPaymentTable tbody").append("<tr class='text-danger' style='font-weight: 800;'><td colspan='1' class='text-right'>合计</td><td colspan='2'>" + sumRatio + "</td></tr>");
	        	
	        	var $emptyTr = $("#dispatchPaymentTable tbody .empty");
                if ($emptyTr.length == 1) {
                    $emptyTr.remove();
                    $(".empty-template").appendTo($("#dispatchPaymentTable tbody")).removeClass("hidden empty-template");
                }
                // 将导出按钮移至最后
                $(".exportlinks", $("#dispatchPaymentForm")).appendTo($("#dispatchPaymentForm"));
                
                var maskMoneyCfg = {defaultZero: true, allowZero:true, precision:2};
	        	$("#dispatchPaymentTable tbody .text-input").each(function(index) {
	        		...
	        		var perfix = "dispatchPaymentList[" + rowNum + "].";
	        	...
	        	function checkPaymentForm() {
                   /*  var isEm = <s:property value='user.isHasRole(13)'/> || false;
                    ...
                    if (!(((<s:property value='user.isHasRole(13)'/> || false) || (<s:property value='user.isHasRole(10)'/> || false))/*  || (state == "0" || state == '') */)) {
                        $("#dispatchPaymentForm input").attr("disabled", true);
                        $("#paymentSubmitBtn").hide();
                        if ($("#applyPaymentBtn:visible").length == 0) {
                        	return false;
                        } else {
                        	$("#dispatchPaymentForm input[type='hidden']").removeAttr("disabled");
                        }
                    }
                    if (state == "100") {
                    	//$("#dispatchPaymentForm input").not("input[type='file']").not("input[type='hidden']").attr("disabled", true);
                    	$("#dispatchPaymentForm input").attr("disabled", true);
                    }
                    if (!calcRatio()) {
                    	return false;
	        	...
	        	$(document).on("submit", "#dispatchPaymentForm", function() {
	        	    if (taskKey == "generateContractTask") {
                        var dispatchNo = $.trim($("#dispatchPaymentForm input[name='dispatch.dispatchNo']").val());
                        if (!dispatchNo) {
                            alert("请输入转包合同号！");
                            var $btn = $(this).find("button");
                            $("#dispatchPaymentForm input[name='dispatch.dispatchNo']").focus();
                            setTimeout(function() {
	        	    ...
	        	    if (("<s:property value='result'/>" || "") && dispatchType == '30') {
	        	...
	        	/* $(document).on("click", "#paymentSubmitBtn", function() {
	        	    $("input[name='workflowCommonParam.approveStatus'][value='5']", $("#dispatchPaymentForm")).prop("checked","checked");
	        	}); */
	        	$(document).on("click", "#approvePaymentBtn", function() {
                    $("input[name='workflowCommonParam.flag']", $("#dispatchPaymentForm")).val("1");
                });
                ...
                $(document).on("change", ".ratio input[name*='ratio']", function(){
                	if (calcRatio()) {
                		var dispatchAmount = $.trim($("#dispatchInfoTable #dispatchAmount").text());
                        dispatchAmount = Number(dispatchAmount.replace(/,/g, ""));
                        
                        var amount = 0;
                        if (!isNaN(dispatchAmount)) {
                            amount = dispatchAmount * this.value / 100;
                        }
                ...
                function calcAmount() {
	        		var sumAmount = 0;
	        		var paiedAmount = 0;
                    $("#dispatchPaymentTable tbody .amount").each(function(){
                ...
                $(document).on("change", "#dispatchPaymentTable tbody .amount", function(){
                	calcAmount();
                });
	        })
        </script>
