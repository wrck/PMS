...
public class PmWorkFlowService extends AbstractBaseService<PmWorkFlowMapper, PmWorkFlow> implements IPmWorkFlowService {
	...
	public List<PmWorkFlow> selectRunTasksByAssigneeAndProcessKeyAndTaskKey(PageParam<PmWorkFlow> pageParam,
			Integer assignee, String processKey, String taskKey) {
		// TaskQuery taskQuery =
		...
		TaskQuery taskQuery = taskService.createTaskQuery().processDefinitionKey(processKey)
				.taskCandidateOrAssigned(assignee.toString()).taskDefinitionKey(taskKey)
				.taskVariableValueLessThanOrEqual("startTime", new Date()).active();// startTime在planStepListener中赋值，取考核步骤的开始时间，默认值当前系统时间
		Integer totalSum = taskQuery.list().size();
		// pageParam.setTotal(totalSum);
		if (pageParam.getPageSize() == -1) {
			pageParam.setPageSize(totalSum);
		}
		List<Task> taskList = taskQuery.orderByTaskCreateTime().asc().listPage(pageParam.getStart(),
				(int) pageParam.getPageSize());
		List<PmWorkFlow> perfWorkFlowList = new ArrayList<PmWorkFlow>();
		for (Task task : taskList) {
			PmWorkFlow perfWorkFlow = (PmWorkFlow) runtimeService.getVariable(task.getExecutionId(), "entity");
			if (perfWorkFlow == null) {
				perfWorkFlow = new PmWorkFlow();
			}
//			PlanVO planVO = new PlanVO();
//			planVO.setId(perfWorkFlow.getPlanId());
//			planVO.setParticipantId(perfWorkFlow.getParticipantId());
...
//			perfWorkFlow.setPlanVO(planVO);
//			perfWorkFlow.setTaskId(task.getId());
//			perfWorkFlow.setAssignee(task.getAssignee());
//			if (StringUtils.isNotBlank(task.getAssignee())) {
//				Employee employee = employeeDao.selectByPrimaryKey(Integer.valueOf(task.getAssignee()));
//				perfWorkFlow.setAssigneeName(employee != null ? (employee.getWorkNo() + "-"  + employee.getName()) : "");
//			} else {
...
//						perfWorkFlow.setAssigneeName(group != null ? group.getName() : "");
//					} else {
//						perfWorkFlow.setAssignee(link.getUserId());
//						Employee employee = employeeDao.selectByPrimaryKey(Integer.valueOf(link.getUserId()));
//						perfWorkFlow.setAssigneeName(employee != null ? (employee.getWorkNo() + "-"  + employee.getName()) : "");
//					}
//				}
//			}
			perfWorkFlow.setFormUrl(task.getFormKey());
			perfWorkFlow.setTaskKey(taskKey);
			perfWorkFlow.setProcInstId(task.getProcessInstanceId());
			perfWorkFlow.setDueTime(task.getDueDate());
			perfWorkFlowList.add(perfWorkFlow);
		}
		pageParam.setTotal(totalSum + pageParam.getTotal());
		return perfWorkFlowList;
	}
...
//			}
			perfWorkFlow.setFormUrl(task.getFormKey());
			perfWorkFlow.setTaskKey(task.getTaskDefinitionKey());
			perfWorkFlow.setProcInstId(task.getProcessInstanceId());
			perfWorkFlow.setDueTime(task.getDueDate());
			perfWorkFlowList.add(perfWorkFlow);
		}
...
//		 taskQuery = taskQuery.taskAssignee(assignee.toString()).endOr().finished();
		...
		for (HistoricTaskInstance task : taskList) {
			HistoricVariableInstance entity = historyService.createHistoricVariableInstanceQuery()
					.executionId(task.getExecutionId()).variableName("entity").singleResult();
			if (entity == null) {
				entity = historyService.createHistoricVariableInstanceQuery()
						.processInstanceId(task.getProcessInstanceId()).variableName("entity").singleResult();
			}
			if (entity == null) {
				continue;
			}
			PmWorkFlow perfWorkFlow = (PmWorkFlow) entity.getValue();
			if (perfWorkFlow == null) {
				perfWorkFlow = new PmWorkFlow();
			}
//			PlanVO planVO = new PlanVO();
...
//			}
			...
			perfWorkFlow.setProcInstId(task.getProcessInstanceId());
			perfWorkFlow.setDueTime(task.getDueDate());
			perfWorkFlow.setEndTime(task.getEndTime());
			Result canWithdraw = processService.canWithdraw(task.getProcessInstanceId(), task.getAssignee());
			perfWorkFlow.setCanWithdraw(canWithdraw.isSuccess());
			perfWorkFlowList.add(perfWorkFlow);
		}
		// pageParam.setTotal(perfWorkFlowList.size());
		return perfWorkFlowList;
	}

	@Override
	public PmWorkFlow currentParticipantWorkFlow(PmWorkFlow perfWorkFlow, Object planParticipant) {
...
//		Integer empID = user.getUserInfoId();
		...
		String taskId = perfWorkFlow.getTaskId();
...
//			List<PmWorkFlow> perfWorkFlowList = this.selectBySelective(workFlow);
//			if (!perfWorkFlowList.isEmpty()) {
//				List<String> processInstanceIds = new ArrayList<>(perfWorkFlowList.size());
//				for (PmWorkFlow temp : perfWorkFlowList) {
...
//			}
			...
			// 当前任务
			TaskInfo task = taskService.createTaskQuery().taskId(taskId).taskCandidateOrAssigned(assignee).active()
					.singleResult();
			if (task == null) {
				// 历史任务
				List<String> candidateGroups = this.getCandidateGroups(empID);
				HistoricTaskInstanceQuery historicTaskQuery = historyService.createHistoricTaskInstanceQuery()
						.taskId(taskId).or().taskAssignee(assignee);
				if (!candidateGroups.isEmpty()) {
...
//				}
	...
	private NativeHistoricTaskInstanceQuery candidateOrAssignedHistoricTaskQuery(Integer assignee, String fuzzy) {
		...
		NativeHistoricTaskInstanceQuery taskQuery = historyService.createNativeHistoricTaskInstanceQuery().sql(
//				"SELECT DISTINCT RES.* FROM ACT_HI_TASKINST RES " + "LEFT JOIN ACT_HI_IDENTITYLINK HI_OR0 "
//						+ "	ON HI_OR0.TASK_ID_ = RES.ID_ "
//						+ "INNER JOIN (SELECT * FROM perf_workflow WHERE id IN (SELECT MAX(id) FROM perf_workflow GROUP BY `planId`, `participantId`)) pw"
//						+ " ON pw.`procInstId` = RES.`PROC_INST_ID_` "
//						+ "WHERE RES.END_TIME_ is not null and ( RES.ASSIGNEE_ = #{assignee} OR ( HI_OR0.`TYPE_` = 'candidate' AND ( HI_OR0.USER_ID_ = #{assignee} OR FIND_IN_SET(HI_OR0.`GROUP_ID_`, #{groupIds}) ) ) ) "
//						+ "GROUP BY RES.`PROC_INST_ID_` ORDER BY RES.END_TIME_ DESC");
				"SELECT * FROM ACT_HI_TASKINST RES INNER JOIN ("
						+ "SELECT DISTINCT MAX(CAST(RES.ID_ AS UNSIGNED)) AS maxID FROM ACT_HI_TASKINST RES "
						+ "	LEFT JOIN ACT_HI_IDENTITYLINK HI_OR0 ON HI_OR0.TASK_ID_ = RES.ID_ "
						+ " INNER JOIN perf_workflow pw ON pw.`procInstId` = RES.`PROC_INST_ID_` "
						+ "	LEFT JOIN `perf_plan` p ON p.`id` = pw.`planId` "
						+ " LEFT JOIN `perf_lookup` lk ON lk.codeType = 'planParticipantStatus' AND lk.state = TRUE "
						+ "		AND lk.code = IF(LOCATE('directSummary', RES.`TASK_DEF_KEY_`) > 0, CONCAT(RES.TASK_DEF_KEY_, '_', RES.`PRIORITY_`), RES.TASK_DEF_KEY_)"
						+ "WHERE RES.END_TIME_ IS NOT NULL AND ( RES.ASSIGNEE_ = #{assignee} OR ( HI_OR0.`TYPE_` = 'candidate' AND ( HI_OR0.USER_ID_ = #{assignee} OR FIND_IN_SET(HI_OR0.`GROUP_ID_`, #{groupIds}) ) ) ) "
						+ fuzzySearchSql
						+ "GROUP BY pw.`planId`, pw.`participantId`) t ON RES.`ID_` = t.maxID ORDER BY RES.END_TIME_ DESC");
		if (!candidateGroups.isEmpty()) {
	...
	public void deleteProcess(final Object planParticipant, final String participantIds) {
		PmWorkFlowVO perfWorkFlowVO = new PmWorkFlowVO();
		perfWorkFlowVO.setParticipantIds(participantIds);
		List<String> procInstIds = this.selectProcInstIdsBySelective(perfWorkFlowVO);
		deleteProcessThread(procInstIds);
	}

	@Override
	public void deleteProcess(final PmWorkFlowVO perfWorkFlowVO) {
		List<String> procInstIds = this.selectProcInstIdsBySelective(perfWorkFlowVO);
		deleteProcessThread(procInstIds);
	}
...
//			tempPwf.setParticipantId(participantId);
			...
			if (count == 0) {
				NativeHistoricTaskInstanceQuery historicTaskInstanceQuery = historyService
						...
						.sql("SELECT * FROM `act_hi_taskinst` WHERE TASK_DEF_KEY_ = #{taskKey} AND (DELETE_REASON_ IS NULL OR DELETE_REASON_ = 'completed') AND FORM_KEY_ = CONCAT('/perf/planParticipant/planList/', #{participantId}, '.html') ORDER BY START_TIME_ DESC LIMIT 1");
	...
	public String startProcess(PmWorkFlow pmWorkFlow, Object entity) {
		...
		ExecutionEntity processInstance = (ExecutionEntity) runtimeService
				.startProcessInstanceByKey(pmWorkFlow.getProcessKey(), businessKey, variables);
