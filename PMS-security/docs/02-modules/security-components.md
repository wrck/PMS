...
| 方法签名 | 说明 |
|----------|------|
| `public static int indexOf(byte[] text, String pattern)` | KMP 算法在字节数组中查找子字符串，返回起始位置（未找到返回 -1） |
| `public static int indexOf(byte[] text, byte[] pattern)` | KMP 算法在字节数组中查找子数组，返回起始位置（未找到返回 -1） |
| `private static int[] computeLPSArray(byte[] pattern)` | 计算 KMP 算法的 LPS（最长前缀后缀）数组 |
| `public static ByteBuffer append(ByteBuffer builder, byte[] bytes)` | 向 DirectByteBuffer 追加字节数组，容量不足时自动扩容（2 倍或所需容量） |
| `private static ByteBuffer expandDirectByteBuffer(ByteBuffer builder, int additionalCapacity)` | 扩展 DirectByteBuffer 容量（创建新 Buffer 并复制数据） |
| `public static byte[] readBytes(ByteBuffer builder)` | 从 DirectByteBuffer 读取字节数组（先 flip 后 get） |
| `public static ByteArrayOutputStream append(ByteArrayOutputStream builder, byte[] bytes) throws IOException` | 向 ByteArrayOutputStream 追加字节数组 |

> **使用场景**：`XssRequestBodyHttpServletRequestWrapper` 处理 multipart 表单时，使用 `ByteUtils.indexOf`（KMP）定位每个 `FileItem` 在原始 Body 中的字节偏移，再用 `append` + `readBytes` 重写 Body。
