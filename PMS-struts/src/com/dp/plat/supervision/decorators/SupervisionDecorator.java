package com.dp.plat.supervision.decorators;
 ...
 */
public class SupervisionDecorator extends TableDecorator {

    public String getExpendSupervisionQuesResult() {
        Object obj = getCurrentRowObject();
        StringBuilder html = new StringBuilder();
        if (obj instanceof ProjectSupervisionVO) {
            Integer maxQuesResultSize = (Integer) getPageContext().getAttribute("MaxQuesResultSize");
            ProjectSupervisionVO projectSupervision = (ProjectSupervisionVO) obj;
            List<Map<String, String>> resultList = projectSupervision.getQuesnaireResultList();
            ...
            Map<String, Object> projectSupervision = (Map<String, Object>) obj;
            for (int i = 1;; i++) {
                String key = "questionResult" + i;
                String nextKey = "questionResult" + (i + 1);
                if (!projectSupervision.containsKey(key)) {
                    break;
                } else {
                    html.append(StringUtils.trimToEmpty((String) projectSupervision.get(key)));
                    if (projectSupervision.containsKey(nextKey)) {
                        html.append("</td><td>");
                    }
                }
            }
    ...
    public String getExpendDeliverFilesURL() {
        ...
        if (obj instanceof ProjectSupervisionVO) {
            ProjectSupervisionVO projectSupervision = (ProjectSupervisionVO) obj;
            List<FileParam> resultList = projectSupervision.getDeliverFileList();
            ...
            Map<String, Object> projectSupervision = (Map<String, Object>) obj;
            String deliverFiles = StringUtils.trimToEmpty((String) projectSupervision.get("deliverFiles"));
    ...
    public String getProjectNameWithURL() {
        ...
        String projectName = "";
        if (obj instanceof ProjectSupervisionVO) {
            projectId = ((ProjectSupervisionVO) obj).getProjectId();
            projectName = ((ProjectSupervisionVO) obj).getProjectName();
        } else if (obj instanceof Map) {
    ...
    public String getOperateUrl() {
        ...
        String createUser = "";
        if (obj instanceof ProjectSupervisionVO) {
            supervisionId = ((ProjectSupervisionVO) obj).getId();
            state = ((ProjectSupervisionVO) obj).getState();
            createUser = ((ProjectSupervisionVO) obj).getCreateBy();
        } else if (obj instanceof Map) {
        ...
        if (Boolean.FALSE.equals(state) && (createUser.equals(user.getUsername()) || (user.isHasRole(MessageUtil.ROLE_ENGINEEMANAGER_LEADER) || user.isHasRole(MessageUtil.ROLE_ENGINEEMANAGER)))) {
            ...
            taskName = "删除";
            url.append("<a href='javascript:void(0)' onclick='").append("deleteSupervision(\"").append(supervisionId).append("\")'>").append(taskName).append("</a>");
        }
        return url.toString();
    }
}
