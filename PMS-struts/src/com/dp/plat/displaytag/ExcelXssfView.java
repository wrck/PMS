...
/**
 * Excel exporter using POI XSSF.
 * 
 * @author Fabrizio Giustina
 * @author rapruitt
 * @version $Revision: 1086 $ ($Author: rapruitt $)
 */
public class ExcelXssfView extends AbstractExcelView implements BinaryExportView
{
	...
	public void doExport(OutputStream out) throws JspException
	{
		try
		{
			SXSSFWorkbook wb = new SXSSFWorkbook(1000);//缓存
			...
			if (this.header)
			{
				...
				SXSSFRow xlsRow = sheet.createRow(rowNum++);
...
//				bold.setBoldweight(HSSFFont.BOLDWEIGHT_BOLD);
				bold.setBold(true);
				bold.setColor(IndexedColors.WHITE.index);
				...
				while (iterator.hasNext())
				{
					...
					if (columnHeader != null && !htmlAttributes.isEmpty() && htmlAttributes.containsValue("splitCell=true")) {
                        ...
                        for (String td : tds) {
                        	SXSSFCell cell = xlsRow.createCell((short) colNum++);
                            XSSFRichTextString riHeader = new XSSFRichTextString(td);
                            cell.setCellValue(riHeader);
                            cell.setCellStyle(headerStyle);
                        }
                    } else {
                    	SXSSFCell cell = xlsRow.createCell((short) colNum++);
    					XSSFRichTextString riHeader = new XSSFRichTextString(columnHeader);
    					cell.setCellValue(riHeader);
    					cell.setCellStyle(headerStyle);
                    }
				}
...
//						String[] tds = StringUtils.splitByWholeSeparatorPreserveAllTokens(String.valueOf(value), "</td><td>");
	...
	protected void writeCell(Object value, Cell cell)
	{
		...
		*/
		...
		{
			XSSFRichTextString richValue = new XSSFRichTextString(escapeColumnValue(value));
	...
	static class ExcelGenerationException extends BaseNestableJspTagException
	{
		...
		{
			super(ExcelXssfView.class, Messages
