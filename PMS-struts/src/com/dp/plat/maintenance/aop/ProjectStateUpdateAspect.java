...
package com.dp.plat.maintenance.aop;
...
@Aspect
@Component
public class ProjectStateUpdateAspect {

    protected final static Logger logger = LoggerFactory.getLogger(ProjectStateUpdateAspect.class);

    private static final String TAG = "【项目状态更新AOP】";
    private static final String PM_PROJECT_STATE_UPDATE_RULES_CONFIG = "pm.project.state.update.rules.config";
    private static final Supplier<Map<String, Object>> configSupplier = new Supplier<Map<String, Object>>() {
        @Override
        public Map<String, Object> get() {
            return SystemContext.getSystemContext().getCacheJsonValue(PM_PROJECT_STATE_UPDATE_RULES_CONFIG, HashMap.class, new HashMap<String, Object>(0));
        }
    };
     ...
     */
    @Pointcut("(target(com.dp.plat.service.ProjectService) && (execution(* com.dp.plat.service.ProjectServiceImpl.insertOrUpdateProjectState(..)))) ||"
            + " execution(* com.dp.plat.service.ProjectServiceImpl.insertOrUpdateProjectState(..)) "
            + " || execution(* com.dp.plat.service.ProjectServiceImpl.updateProjectExecutionState(..))")
    ...
    public Object updateProjectStateAdvice(ProceedingJoinPoint joinPoint) throws Throwable {
        // 方法执行前的逻辑
        log("开始");
        ...
        try {
            ...
            Map<String, Object> paramsMap = AspectUtil.getParamsMap(joinPoint, paramsRelation);
            this.paramRelations.set(paramsRelation);
            this.params.set(paramsMap);
    
            updateProjectStateBefore(joinPoint);
            
            // 执行目标方法
            Object result = joinPoint.proceed();
    
            updateProjectStateAfter(joinPoint);
...
//    @AfterReturning(pointcut = "serviceAspect()", returning = "result")
    public void updateProjectStateAfterReturning(JoinPoint joinPoint, Object result) {
...
//    @Before("serviceAspect()")
    public void updateProjectStateBefore(JoinPoint joinPoint) {
        log("前置条件触发开始");
        try {
            updateProjectStateByRule(joinPoint, "before");
        } finally {
            log("前置条件触发结束");
        }
    }
     ...
     */
//    @After("serviceAspect()")
    public void updateProjectStateAfter(JoinPoint joinPoint) {
        log("后置条件触发开始");
        try {
            updateProjectStateByRule(joinPoint, "after");
        } finally {
            log("后置条件触发结束");
        }
    }
     ...
     * @param joinPoint 切点对象
     */
    public void updateProjectStateByRule(JoinPoint joinPoint, String type) {
        try {
            ...
            if (config != null && !config.isEmpty()) {
                ...
                Project project = (Project) paramsMap.get("project");
                if (project == null) {
                    return;
                }
                ...
                execScripts(project, typeRules);
//                projectService.updateProjectByProjectIdSelective(project);
            }
        } catch (Exception e) {
     ...
     * @return
     */
    public Object execScripts(Project project, Map<String, Object> config) {
        return execScripts(project, null, config, null);
    }
     ...
     * @return
     */
    public Object execScripts(Project project, Map<String, Object> config, String scriptName) {
        return execScripts(project, null, config, scriptName);
    }
     ...
     * @return
     */
    public Object execScripts(Project project, Map<String, Object> entity, Map<String, Object> config, String scriptName) {
        if (entity == null) {
    ...
    public void logError(String format, Object... arguments) {
        format = new StringBuilder("{}-").append(format).toString();
        logger.error(format, getTag(), arguments);
    }
   ...
   public void log(String format) {
    ...
    public void log(String format, Object... arguments) {
        ...
        format = new StringBuilder("{}-").append(format).toString();
        if (debug) {
            logger.debug(format, getTag(), arguments);
        }
    }
    ...
    public void log(String format, boolean isDebug, Object... arguments) {
        ...
        if (debug && isDebug) {
            ...
            logger.info(format, getTag(), arguments);
        }
    }
