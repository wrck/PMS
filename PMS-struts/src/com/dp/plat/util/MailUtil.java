...
import com.dp.plat.util.test.MyAuthenticator;
...
public class MailUtil {
	 ...
	 * @param mailInfo
	 */
	public static void keepMail(MailSenderInfo mailInfo) {
		// List<MailSenderInfo> mailInfos = sortMailType(mailInfo);
		// for (MailSenderInfo info : mailInfos) {
	...
	public static void keepMail(Map<String, Object> context, boolean isSendNow) {
		...
		info.setTos((String) context.get("tos"));
		info.setCcs((String) context.get("ccs"));
		info.setBcc((String) context.get("bccs"));
		info.setSubject((String) context.get("subject"));
		String content = (String) context.get("content");
		if (StringUtils.isBlank(content)) {
			content = "";
		...
		info.setContent(content);
		info.setAttachFileNames((String) context.get("attachFiles"));
		info.setMailExpectSendTime((Date) context.get("expectSendTime"));
		info.setCreateBy((String) context.get("createBy"));
	 ...
	 *            是否立即发送
	 */
	public static void keepMail(MailSenderInfo mailInfo, boolean isSendNow) {
		List<MailSenderInfo> mailInfos = sortMailType(mailInfo);
		for (MailSenderInfo info : mailInfos) {
			if (isSendNow) {
...
//			ExceptionHandler.insertException(e);
	...
	private static boolean sendMail(Message mailMessage, MailSenderInfo mailInfo) {
		try {
			List<Message> mailMessageList = sortMailType(mailMessage, false);
...
//		}
	...
	private static MimeMessage createMimeMessage(MailSenderInfo mailInfo) throws MessagingException {
		...
		Address from = addNickForAddr(StringEscUtil.getText("sys.title"), mailInfo.getFromAddress());
		// 设置邮件消息的发送者
...
//					ExceptionHandler.insertException(e);
	...
	private static String[] findTemplate(Map<String, Object> context) {
		...
		return new String[] { template.getNotificationSubject(), template.getNotificationContent() };
	}
...
//		String envirmentArgu = systemVariables.get("sys.envirment.argu");
		String envirmentArgu = "0";
		try {
			BasicDataDao basicDataDao = (BasicDataDao) SpringContext.getBean("basicDataDao");
	        envirmentArgu = basicDataDao.querySysArg("sys.envirment.argu");
...
//		mailInfo.set(actualSendAddress);
	 ...
	 */
	private static Multipart addAttachments(MailSenderInfo mailInfo, Multipart multipart)
...
//				}
	 ...
	 * @throws MessagingException
	 */
	private static void insertPictureIntoContent(MailSenderInfo mailInfo, MimeBodyPart content) throws MessagingException {
	 ...
	 * @return
	 */
	private static List<MailSenderInfo> sortMailType(MailSenderInfo info) {
		int mailType = checkMailType(info);
		List<MailSenderInfo> mailInfos = new ArrayList<>();
		if (mailType == 1) {
...
//			ExceptionHandler.insertException(e);
	 ...
	 */
	private static int checkMailType(MailSenderInfo info) {
		String[] mailStrs = new String[] { info.getTos(), info.getCcs(), info.getBcc() };
...
//			ExceptionHandler.insertException(e);
	 ...
	 * @return mainInfo
	 */
	private static MailSenderInfo cloneMainInfo(MailSenderInfo info, StringBuilder tos, StringBuilder ccs, StringBuilder bccs,
			boolean isInner) {
		MailSenderInfo mailInfo = new MailSenderInfo();
		BeanUtils.copyProperties(info, mailInfo);
		mailInfo.setTos(tos.toString());
		mailInfo.setCcs(ccs.toString());
		mailInfo.setBcc(bccs.toString());
		if (isInner) {
//			mailInfo.setIsInner(isInner);
		}
		return mailInfo;
