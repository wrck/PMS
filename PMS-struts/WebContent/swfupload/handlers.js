...
function fileQueueError(file, errorCode, message) {
	try {
		...
		case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
			...
			progress.setStatus("<font color='red'>文件大小为0，上传失败</b></font>");
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			...
			var progress = new FileProgress(file,  this.customSettings.upload_target);
			progress.setComplete();
			progress.setStatus("<font color='red'>文件大小超过限制，上传失败</b></font>");
			progress.toggleCancel(false);
			break;
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			var progress = new FileProgress(file,  this.customSettings.upload_target);
			progress.setComplete();
			progress.setStatus("<font color='red'>文件上传类型识别不了，上传失败</b></font>");
			progress.toggleCancel(false);
			break;
		default:
			var progress = new FileProgress(file,  this.customSettings.upload_target);
			progress.setComplete();
			progress.setStatus("<font color='red'>文件上传失败，请重试</b></font>");
			progress.toggleCancel(false);
			break;
		}
	} catch (ex) {
...
//		addFileId(file.id,serverData);
		...
		$("#"+(file.id).substring(5)).text(serverData);
//		var name = $("#responsibleName").value();
	} catch (ex) {
		this.debug(ex);
	}
}
...
//	var second = now.getSeconds().toString();
	...
	var trid = fileid.substring(3);
	var tdid = fileid.substring(5);

	//获取浏览器的类型
	//var oType = getBrowserType();

	var html = '<tr id="'+trid+'"><td style="height:25px;border-right:1px solid #888;border-bottom:1px solid #888;padding:0.5px 15px;text-align:center;">'+fileName+'</td>'+
		'<td style="border-right:1px solid #888;border-bottom:1px solid #888;padding:0.5px 15px;text-align:center;">'+name+'</td>'+
		'<td style="border-right:1px solid #888;border-bottom:1px solid #888;padding:0.5px 15px;text-align:center;">'+time+'</td>'+
		'<td style="border-right:1px solid #888;border-bottom:1px solid #888;padding:0.5px 15px;text-align:center;"><a href="javascript:deleteFile('+tdid+')">删除</a></td><td style="display:none" id="'+tdid+'" ><span ></span></td></tr>';
