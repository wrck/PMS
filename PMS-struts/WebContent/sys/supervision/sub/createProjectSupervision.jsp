...
<script type="text/javascript">
	$(function() {
		$("#pmCLChoseQuesButt").change(function() {
			$("#projectSupervisionForm #supervisionProjectId").val(null);
			$("#projectSupervisionForm").submit();
		...
		$("#submitButton").click(function(){
			for (var i = 0; i < $(".tag-must").length; i++) {
				...
				var $inpt = $(_this).next().find("[name*='projectSupervision.'], #pmCLChoseQuesButt");
		...
		uploadCallback("${projectSupervision.deliverFileIds}");
	});
	...
	function uploadTaskFile(projectId ,taskId){
        ...
        var column010 = "projectSupervision";
        var column011 = "";//$("#projectType").val();
        var contractNo = '${project.contractNo}';
        var eventKey = "supervisionTask-create";
...
<body>
    <s:form id="projectSupervisionForm" name="projectSupervisionForm" action="supervision_createProjectSupervision.action" method="post" class="supervisionForm">
	<div>
	    <%-- <s:if test="subcontractCallback.quesnaireState == 1"> --%>
	    <div class="panel panel-default">
		    <div class="panel-body">
                    <s:hidden id="projectId" name="project.projectId"></s:hidden>
                    <%-- <s:hidden id="supervisionId" name="projectSupervision.id"></s:hidden> --%>
                    <s:hidden id="supervisionProjectId" name="projectSupervision.projectId" value="%{project.projectId}"></s:hidden>
                    <s:hidden name="redirect"/>
                    
                    <!-- 项目基本信息 -->
                    <table id="projectSupervisionTable" class="table table-bordered table-hover table-striped noBorder">
                        <tr>
                        ...
                        <tr>
                            <td class="tag tag-must"><s:text name="pm.project.supervision.type"></s:text>:</td>
                            <td>
                                <%-- headerValue="--请选择--" headerKey="" --%>
                                <s:select list="supervisionTypeList" id="supervisionType" name="projectSupervision.type" listKey="basicDataId" listValue="basicDataName" cssClass="form-control" cssStyle="width: 180px;display: inline-block;" />
                            </td>
                            <td class="tag tag-must"><s:text name="pm.project.supervision.processTime"></s:text>:</td>
                            <td>
                                <s:textfield name="projectSupervision.processTime" id="processTime" placeholder="处理时间" cssClass="form-control"/>
                            </td>
                        </tr>
                        <%-- <tr>
                            <td class="tag tag-must"><s:text name="pm.project.supervision.state"></s:text>:</td>
                            <td><s:radio list="#{false:'无', true:'有'}" name="projectSupervision.state"/></td>
                            <td class="tag"><s:text name="pm.remark"></s:text>:</td>
                            <td><s:textarea name="projectSupervision.remark" cssClass="form-control"/></td>
                        </tr> --%>
                        
                        <tr>
                            <td class="tag tag-must"><s:text name="pm.project.supervision.channel"></s:text>:</td>
                            <td><s:textarea name="projectSupervision.channel" cssClass="form-control"/></td>
                            ...
                            <td>
                                <s:hidden name="projectSupervision.deliverFileIds" id="deliverFileIds"/>
                        ...
                        <tr>
                            ...
                            <td><s:textarea name="projectSupervision.remark" cssClass="form-control"/></td>
                            <td colspan="2">
                                <s:if test="projectSupervision == null">
                                    <div class="text-left">
	    ...
	    <%-- </s:if>
	    <s:else> --%>
        <s:if test="%{projectSupervision.state == false && (user.isHasRole(10) || user.isHasRole(13) || user.isHasRole(14))}">
		...
		<%-- </s:else> --%>
		<s:if test="projectSupervision != null && cbForm.pmClosedLoopQuesnaireLineList.size()>0">
					...
					<div class="panel panel-default">
						...
						<div id="cbcollapseOne" class="panel-collapse collapse in">
							<div class="panel-body">
								...
								<%-- <s:form method="post" action="querySubcontractCallback.action" id="cbCLForm" class="supervisionForm"> --%>
									...
									<s:hidden name="projectSupervision.id"></s:hidden>
									<s:hidden name="pmClQuesnaireResultHeader.status" value="-1" id="quesnaireState"></s:hidden>
									<!-- 问卷已提交 -->
									<s:if test="projectSupervision.quesnaireState == 1">
										<div> 
											<div class="pmclnotice">
