...
function callback(){
	...
	popWindow('module/sub/presales_callback.action?pmClosedLoopQuesnaire.id=3&redirect='+href+'&presales.presalesId='+presalesId, 900, 650,'回访问卷', 'BudgetUpload', true);
	return false;
}
function shipmentInfo() {
	var presalesCode = $("input[type='hidden'][name='presales.presalesCode']").val();
	$.ajax({
		url:"module/presales_shipmentInfo.action",
		type:"post",
		dataTpe:"html",
		data:{"presalesCode":presalesCode},
		success: function(data) {
			data = data.substring(data.indexOf("<fieldset>"), data.indexOf("</fieldset>") - 11);
			$("#product").after(data);
		}
	})
}
...
<body>
	<fieldset>
			...
			<tr>
				...
				<td>
                    ...
                    <s:if test="presales.hasTransfer == 1">
                        <span class="text-danger">(<s:text name="pm.presales.hasTransfer"/>)</span>
                    </s:if>
                </td>
			</tr>
			<tr>
        ...
        <table class="table table-no-border" style="margin-top: -20px;margin-bottom: 0;">
            <tr>
                <td <s:if test='presales.projectState == 30'>class="text-success current-state"</s:if>><s:text name="pm.presales.serviceDuration"></s:text>:<s:property value="presales.serviceDuration"/></td>
