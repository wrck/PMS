    ...
    $(function(){
       ...
       /*  queryPowerUser();
        ...
        function popWindow(url, width, height, title, dialogid, modal) {
        	...
        	var searchKV = search.split("&");
            $(".ignore").removeAttr("disabled");
        	search = searchKV.filter(function(str) {
        		var filter = !str.endsWith("=");
        		return filter;
        	}).join("&");
        	
        	win_popWindow(url, width, height, title, dialogid, modal);
        }
    });
    function updateProject(obj){
        window.open("${namespace}/ProjectModify.action?project.paramId="+obj + "&result=314");
    }
    win_popWindow = window.popWindow;
    window.popWindow = function(url, width, height, title, dialogid, modal) {
		$(".ignore").attr("disabled", true);
    	var search = $("#mainForm").serialize();
    	var searchKV = search.split("&");
        $(".ignore").removeAttr("disabled");
    	...
    	win_popWindow(url + "?" + search, width, height, title, dialogid, modal);
    }
    /* var usernameArr = [];
    var realnameArr = [];
...
<body>
    <!-- 查询 -->
    <s:form id="mainForm" name="mainForm" cssClass="form-inline" action="%{namespace}/warrantyCallback.action" style="width: calc(100vw - 200px);">
    ...
    <div>
        ...
        <display:table id="warrantyCallbackList" class="table table-striped table-title-nowrap"
            ...
            size="${displayParam.totalcount}" sort="external" export="true"  requestURI="${namespace}/warrantyCallback.action" 
            ...
            <%-- <display:column title="客户联系人">
            	<div class="nowrap">PMS客户:${warrantyCallbackList.customer1}<${warrantyCallbackList.customerContact1}></div>
            	<div class="nowrap">SMS客户：${warrantyCallbackList.customer2}<${warrantyCallbackList.customerContact2}></div>
            </display:column> --%>
            ...
            <%-- <display:column property="expendDeliverFiles" title="项目维保回访表" media="excel"></display:column> --%>
            ...
            <display:setProperty name="export.excel.filename" value="${project.projectName}项目维保回访记录.xls" />
        </display:table>
    </div>
    </div>
</body>
