	...
	$(function() {
		...
		$("#submitButton").click(function(){
			for (var i = 0; i < $(".tag-must").length; i++) {
				...
				var $inpt = $(_this).next().find("[name*='projectMaintenance.']");
				var name = $inpt.attr("name");
				var type = $inpt.attr("type");
                var $target = $("[name='" + name + "']" + (type == 'radio' ? ":checked" : ""));
	...
	<div>
	    ...
	    <div class="panel panel-default">
		    <div class="panel-body">
                <s:form id="projectMaintenanceForm" name="projectMaintenanceForm" action="projectSub_createProjectMaintenance.action" method="post" class="maintenanceForm">
                    <s:hidden id="projectId" name="project.projectId"></s:hidden>
                    <s:hidden name="projectMaintenance.projectId" value="%{project.projectId}"></s:hidden>
                    <!-- 项目基本信息 -->
                    <table id="projectMaintenanceTable" class="table table-bordered table-hover table-striped noBorder">
                        ...
                        <tr>
                            <td class="tag tag-must"><s:text name="pm.project.maintenance.type"></s:text>:</td>
                            <td>
                            ...
                            <td>
                                <s:textfield name="projectMaintenance.processTime" id="processTime" placeholder="处理时间" cssClass="form-control"/>
                            </td>
                        </tr>
                        <tr>
	    ...
	    <div id="quesnairePanel" >
	 		<div class="panel panel-default">
				<div class="panel-body">
					<s:form method="POST" action="projectSub_createProjectMaintenance.action"
		...
		<%-- </s:else> --%>
		<s:if test="cbForm.pmClosedLoopQuesnaireLineList.size()>0">
							...
							<div class="panel-body">
								...
								<s:form method="post" action="querySubcontractCallback.action" id="cbCLForm" class="maintenanceForm">
									...
									<s:iterator value="cbForm.pmClosedLoopQuesnaireLineList" id="obja"
										status="indexa">
										<input type="hidden"
											name="cbForm.pmClQuesnaireResultLineList[<s:property value="%{#indexa.index}"/>].quesnaireTemplateLineId"
											value="<s:property value="%{#obja.id}"/>" />
										<input type="hidden"
											name="cbForm.pmClQuesnaireResultLineList[<s:property value="%{#indexa.index}"/>].quesTemplateLineNum"
											value="<s:property value="%{#obja.questionNum}"/>" />
										<input type="hidden"
											name="cbForm.pmClQuesnaireResultLineList[<s:property value="%{#indexa.index}"/>].quesTypeForCB"
											...
											<div class="content_pm_proplem">
												...
												<div class="yl_one_item">
													<table class="yl_one_item_tbl">
														<tbody>
															<tr>
																<s:iterator value="cbForm.pmClosedLoopQuesnaireOptList"
																						...
																						<s:if
																							...
																							<s:if test="#objRLine.quesEvaResult==-1">
																								<td><input type="radio" checked="checked"
																									value="<s:property value="%{#objOpt.id}"/>"
																									id="cbopt_<s:property value="%{#obja.id}"/>_<s:property value="%{#objOpt.questionOptionNum}"/>"
																									name="cbForm.pmClQuesnaireResultLineList[<s:property value="%{#indexa.index}"/>].questionTemplateOptId"
																									style="margin-right: 10px;"></td>
																...
																<s:if test="#obja.questionType==3">
																	<s:if test="cbForm.pmClQuesnaireResultLineList==null">
																		<td><textarea
																				name="cbForm.pmClQuesnaireResultLineList[<s:property value="%{#indexa.index}"/>].questionAnswer"
