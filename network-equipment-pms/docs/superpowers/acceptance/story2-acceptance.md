...
权限码 `project:advance:phase` — 注意 V72 注册的是 `project:phase:advance`，
命名顺序不一致（详见 TD-P8-004）。OK（路径）/ WARN（权限码）
...
| 事务边界 | **PASS** |
| 权限码命名 | **WARN**（TD-P8-004，前端 controller 用 project:advance:phase，V72 用 project:phase:advance） |

**场景 2.1 评分**：8 PASS + 3 WARN。**条件通过** — DELIVERABLE/MILESTONE 拦截正常，
TASK/APPROVAL 拦截缺失（TD-P8-005），权限码命名不一致（TD-P8-004）。
