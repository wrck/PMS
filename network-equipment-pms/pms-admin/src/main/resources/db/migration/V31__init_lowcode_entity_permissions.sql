...
-- 注意：V29__init_lowcode_entity_tables.sql 已注册"实体设计器"菜单（sort_order=2，permission=lowcode:entity:list）。
-- 本脚本仅补充"版本历史"菜单 + 全部按钮权限 + 动态数据权限模板 + 管理员角色授权，避免重复插入。

-- 版本历史页面菜单（实体设计器菜单已在 V29 注册，sort_order=2，此处版本历史用 sort_order=3 保持顺序）
