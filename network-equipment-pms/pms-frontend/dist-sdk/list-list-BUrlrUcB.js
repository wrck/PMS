...
import { C as deleteList, D as exportList, U as listLists, m as archiveList, q as publishList } from "./lowcode-F-suzo7c.js";
...
//#region src/views/lowcode/list-list/index.vue?vue&type=script&setup=true&lang.ts
var _hoisted_1 = { class: "list-list-page" };
...
//#endregion
//#region src/views/lowcode/list-list/index.vue
var list_list_default = /*#__PURE__*/ _plugin_vue_export_helper_default(/* @__PURE__ */ defineComponent({
	__name: "index",
	setup(__props) {
		/**
		* 低代码列表配置列表页。
		*
		* <p>提供列表配置的分页查询、新建（跳转到设计器）、编辑、删除、发布、归档、导入、导出。</p>
		*/
		...
		async function loadData() {
			loading.value = true;
			try {
				const res = await listLists(query);
		...
		function handleAdd() {
			router.push("/lowcode/list-designer");
		}
		...
		async function handleDelete(row) {
			if (!row.id) return;
			try {
				await ElMessageBox.confirm(`确认删除列表「${row.name}」？`, "确认", { type: "warning" });
		...
		async function handleArchive(row) {
			if (!row.id) return;
			try {
				await ElMessageBox.confirm(`确认归档列表「${row.name}」？`, "确认", { type: "warning" });
				await archiveList(row.id);
		...
		return (_ctx, _cache) => {
			...
			return openBlock(), createElementBlock("div", _hoisted_1, [createVNode(_component_el_card, {
				...
				header: withCtx(() => [createElementVNode("div", _hoisted_2, [_cache[9] || (_cache[9] = createElementVNode("span", null, "列表配置列表", -1)), createVNode(_component_el_button, {
					type: "primary",
					icon: "Plus",
					onClick: handleAdd
				}, {
					default: withCtx(() => [..._cache[8] || (_cache[8] = [createTextVNode("新建列表", -1)])]),
					_: 1
				})])]),
...
}), [["__scopeId", "data-v-06ff26bb"]]);
//#endregion
export { list_list_default as default };

//# sourceMappingURL=list-list-BUrlrUcB.js.map