...
import { getTaskDonePage, type TaskRespVO } from '@/api/yudao-bpm'
...
async function loadData() {
  loading.value = true
  try {
    const res = await getTaskDonePage(query)
...
function handleDetail(row: TaskRespVO) {
  ElMessageBox.alert(
    `任务名称：${row.name}<br/>任务编号：${row.id}<br/>流程实例：${row.processInstanceName || row.processInstanceId}<br/>状态：${statusLabel(row.status)}<br/>审批意见：${row.reason || '-'}`,
    '任务详情',
