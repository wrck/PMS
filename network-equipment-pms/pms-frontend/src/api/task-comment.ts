 ...
 * 挂载在 {@code /api/implementation/task/comment} 下，支持二级回复。
 */
...
export function listComments(taskId: number): Promise<TaskCommentItem[]> {
  return get<TaskCommentItem[]>(`/api/implementation/task/comment/${taskId}`)
}

/** 新增评论 */
export function createComment(data: TaskCommentItem): Promise<TaskCommentItem> {
  return post<TaskCommentItem>('/api/implementation/task/comment', data)
}

/** 删除评论 */
export function deleteComment(id: number): Promise<void> {
  return del<void>(`/api/implementation/task/comment/${id}`)
}
