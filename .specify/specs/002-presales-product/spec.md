...
7. **Given** 任一审批节点,**When** 审批结果为驳回,**Then** 流程回退到上一节点。
8. **Given** 代码中存在 serviceApprove 节点键用于服务经理审批时长统计(待澄清,见 Open Questions),**When** 统计时长,**Then** 按 serviceApprove 节点 DURATION 求和计入 serviceApproveDuration。
...
4. **Given** 受影响设备列表已勾选,**When** 发布修复任务,**Then** 批量插入修复任务记录;若指派人为空则角色设为服务经理,否则角色为指定人;状态为 0 时置为开始流程(10)。
...
4. **Given** 办理人更新任务状态,**When** 提交,**Then** 创建流转过程记录并回写 processId 到修复任务主表。
...
8. **Given** 三表 UNION 查询数据量大时性能需评估(待澄清,见 Open Questions),**When** 检索,**Then** 需评估索引优化或缓存策略。
...
#### 待澄清事项(Open Questions)

- **[待澄清] projectState 取值字典**: 代码出现 10/20/30/31/32/33/100 等值,完整状态机与基础数据 27 的映射需核对 fnd_basic_data 实际数据。
- **[待澄清] serviceApprove 节点**: 代码中存在 serviceApprove taskDefKey(用于时长统计),但 Presales.bpmn 流程定义中未见该节点,疑为动态加入或历史遗留。
- **[待澄清] 软件版本检索性能**: 三表 UNION 查询数据量未知,是否需要索引优化或缓存待评估。
- **[待澄清] 设备日志解析策略**: LegacyVersionParserStrategy 与 NewVersionParserStrategy 的匹配规则与适用场景未完全展开,需结合 DeviceLogParserFacade 进一步分析。
- **[待澄清] assigneeRole 角色值**: prob_restore.assigneeRole 取 0 或 MessageUtil.ROLE_SERVICEMANAGER 值,具体数值常量需核对 MessageUtil。
...
- **FR-PRESALES-03: 售前审批流转(基于 BPMN 流程)**
  ...
  - **处理规则**(流程定义见 Presales.bpmn):
    ...
    5. 代码中存在 serviceApprove 节点键(用于服务经理审批时长统计,证据:sql-map-presales-config.xml:806),与 usertask2 关联 [待澄清]。
  - **输出**: 流程实例推进,任务办理人变更。
...
- **FR-PROB-06: 受影响项目检索与修复任务发布**
  ...
  - **处理规则**:
    ...
    6. 若指派人为空,则 assigneeRole=服务经理角色;否则 assigneeRole=0(证据:ProbManageAction.java:480-484)。
    7. restoreStatus=0 时设为 10(开始流程)(证据:ProbManageAction.java:485-486)。
  - **输出**: 修复任务记录;异常返回错误页。

- **FR-PROB-07: 修复任务管理(个人/管理员)**
  - **处理规则**:
    1. 非管理员只能看自己/办事处权限内任务(areapower 注入)(证据:ProbManageAction.java:355-371)。
    2. 个人任务:restoreStatus=10(已发布接受)起查(证据:ProbManageAction.java:524)。
...
> 注:restoreStatus/restoreRemark 不在此表,而在 prob_restore_process(通过 processId 关联)(证据:sql-map-prob-config.xml:836-838)。
...
- **SC-004(源自 NFR-PERF-04 UNION 去重)**: 软件版本检索从三表(出厂版本/项目设备版本/公告版本字典)UNION 去重查询;数据量大时的索引优化与缓存策略需评估 [待澄清]。
...
- **SC-017(源自 NFR-VER-01 版本正则)**: 软件版本必须符合 10 段顺序结构正则;不匹配时按兜底策略处理。
- **SC-018(源自 NFR-VER-02 字典序)**: 版本范围匹配使用字符串 BETWEEN,数字部分补零至 3 位保证字典序与数值序一致。
- **SC-019(源自 NFR-VER-03 范围继承)**: 当固定前缀与范围两端缺省同时存在时,固定前缀优先继承。
- **SC-020(源自 NFR-VER-04 日志解析多策略)**: 设备日志版本解析支持 Legacy 与 New 两种策略,按输入匹配选择 [待澄清]。
...
- **SC-023(源自 NFR-INT-03 流程引擎)**: 售前流程依赖流程引擎运行时任务与历史任务表,通过 instId 关联;时长统计依赖历史任务表的 DURATION 字段。
