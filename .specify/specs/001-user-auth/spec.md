...
- **FR-001: 账号密码登录(非 CAS 模式)**
  ...
  - **处理规则**:
    ...
    6. 计算区域权限(areapower):若为 `-1` 且 dpNo 非空,用 dpNo 替换;若 areapower 不含 dpNo,则追加 dpNo。[暂定决策:倾向 16x↔31x 前缀互换规则已禁用,新系统不实现该互换,仅保留 dpNo 合并逻辑;若业务反馈区域权限缺失再补回。需查 UserUtil.processAreaPower 源码确认,建议业务确认]
...
- **FR-007: 编辑用户**
  ...
  - **处理规则**:
    ...
    4. 若提供 changeType(service/program/both)与 newMemberCode,联动批量更新项目服务经理/项目经理。[暂定决策:changeType=service 更新该用户任服务经理的所有项目(服务经理字段替换为 newMemberCode);program 同理对项目经理;both 两者都更新;联动失败时不回滚用户编辑,仅记录日志。新系统建议改为异步消息。需确认 changeType 三值与项目字段的精确映射,建议业务确认]
...
- **FR-013: 新增角色**
  ...
  - **处理规则**:
    ...
    2. 默认页面 defaultPage 作为角色表单可选输入(不再硬编码 `module/Welcome1.action`),留空时回退至系统默认页。
...
| menuCode | varchar | 否 | 菜单编码(业务键,用户菜单关联用) | 唯一(DDL 层约束,新系统显式建唯一索引) | C |
...
> 已知 code:`sys.cache.latest.refreshTime`(缓存刷新时间)、`sys.envirment.argu`(环境参数,1=生产)、`sys.cas`(CAS 开关)、`sys.cas.server.url`(CAS 服务器地址)。
...
| sendFlag | int | 是 | 发送标志:0=未发,1=已发,2=失败(新系统扩展) | 值域 {0,1,2} | C |
...
> 注:日志查询 LEFT JOIN `user` 表(老表名,与 fnd_user_info 并存)。user 为 fnd_user_info 的同义视图/别名,指向同一物理表;新系统统一为单一用户表,迁移期用视图别名过渡。
...
> **已澄清**:代码中同时存在 `fnd_user_info` 与 `user` 两张用户表,部分 SQL(日志查询、委派查询、MD5 密码更新)仍使用老表名 `user`。决策:user 为 fnd_user_info 的同义视图/别名,指向同一物理表;新系统统一为单一用户表,迁移期用视图别名过渡。
...
> **已澄清**:查询角色名/角色 ID 仍用老表 `role` 而非 `fnd_roles`。决策:role 为 fnd_roles 的别名/视图,新系统统一为 fnd_roles,迁移期用视图兼容。
...
- **SC-008(对应 NFR-010 CAS 可开关)**: CAS 单点登录通过系统参数 `sys.cas`(0=关,1=开)控制;关闭时回退至账号密码登录,开关切换无需改代码。CAS 服务器地址通过系统参数 `sys.cas.server.url` 读取,支持多环境部署。
- **SC-009(对应 NFR-011 邮件可靠性)**: 账号开通/密码重置邮件 100% 持久化到邮件队列;按期望发送时间(mailExpectSendTime<now)轮询发送,发送后 sendFlag=1 且 mailSendTime=now。固定次数重试,达到上限后 sendFlag=2(失败)并告警。
...
- **SC-013(对应 NFR-016 角色变更联动)**: 编辑用户时若指定 changeType(service/program/both)与 newMemberCode,联动批量更新项目服务经理/项目经理。[暂定决策:changeType=service/program/both 分别更新服务经理/项目经理/两者,字段替换为 newMemberCode;联动失败不回滚用户编辑,仅记录日志;新系统建议改异步消息。需确认 changeType 三值与项目字段精确映射,建议业务确认]
- **SC-014(对应 NFR-017 区域权限合并)**: 登录时区域权限(areapower)按规则合并:若 areapower 为 `-1` 且 dpNo 非空,以 dpNo 替换;若 areapower 不含 dpNo,追加 dpNo。[暂定决策:倾向 16x↔31x 前缀互换规则已禁用,新系统不实现该互换;若业务反馈区域权限缺失再补回。需查源码确认,建议业务确认]
...
- **CAS 为生产默认登录方式**: 生产环境默认启用 CAS(`sys.cas=1`),账号密码登录为非 CAS 或回退路径;CAS 服务器地址通过系统参数 `sys.cas.server.url` 配置(参数化)。
- **环境开关语义**: `sys.envirment.argu=1` 表示生产环境(启用验证码且校验密码);非 1 表示测试环境(用户名必须存在有效,系统跳过密码比对,密码任意)。
- **密码加密可演进**: 老架构固定 MD5(password+username);新架构支持可配置加密。新系统不应被 MD5 强约束,但迁移期需兼容历史密码校验。
- **多租户为新架构能力**: 老架构无 compId 隔离;新架构引入公司隔离。spec 同时覆盖两套语义,新系统应以多租户隔离为准(UserRole 表 + compId)。
- **老表(user/role)为同义视图/别名**: 老用户表 `user` 与老角色表 `role` 为 `fnd_user_info`/`fnd_roles` 的同义视图/别名(指向同一物理表),新系统统一为新表,迁移期用视图别名过渡。
- **角色默认页面可配置**: 角色新增/编辑时 defaultPage 作为表单可选输入(不再硬编码 `module/Welcome1.action`),留空时回退至系统默认页。
- **SAP 主数据可访问**: 部门 SAP 同步依赖外部 SAP 视图 `dp_reports.dp_v_spms_department` 可达;同步采用全量 TRUNCATE + 重写,期间存在短暂空窗。
- **邮件发送为异步队列模式**: 通知邮件通过队列表持久化后轮询发送,非实时;发送失败固定次数重试,达到上限后标记失败(sendFlag=2)并告警。
- **缓存为本地 LRU**: 用户/权限缓存与基础数据缓存为本地 LRU(容量 50,只读),不跨节点共享;多实例部署下缓存一致性需新系统重新评估。
- **区域权限映射规则倾向已禁用**: 市场/用服办事处 16x↔31x 前缀互换规则倾向已禁用,新系统不实现,仅保留 dpNo 合并逻辑;若业务反馈区域权限缺失再补回(暂定,建议业务确认)。
- **用户角色变更联动为外部调用**: changeType+newMemberCode 触发的项目服务经理/项目经理批量更新依赖外部项目工具;联动失败不回滚用户编辑仅记录日志;新系统建议改为异步消息(暂定,建议业务确认)。
- **menuCode 唯一约束**: menuCode 在 DDL 层有唯一约束,新系统显式建唯一索引。
- **双授权模型并存**: 用户菜单授权(直接授权)与用户角色关联(间接授权)均为契约,新系统同时支持,统一端点为 REST 风格。
- **menuPower 编码待源码确认**: menuPower 权限编码(`8`=新增/`1`=删除/`4`=查询/`2`=更新)语义反直觉,需查 LoginServiceImpl.java:63-77 源码确认;clarify 前以代码源值为准,新系统建议标准化为位运算(暂定,建议业务确认)。
