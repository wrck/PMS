        ...
        <s:form enctype="multipart/form-data" id="subcontractPaymentForm" action="%{namespace}/subcontract_savePayment.action">
            <table id="subcontractInfoTable" class="table table-bordered table-hover table-striped ">
                ...
                <tr>
                    ...
                    <td class="col-sm-3"><s:textfield name="subcontract.subcontractNo" cssClass="form-control" placeholder='请输入转包合同号'/></td>
                    ...
                    <td>
                        <div class="display-inline-flex">
	                        <s:textfield type="text" name="subcontract.customStrInfo.subcontStartDate" id="subcontStartDate" cssClass="form-control" style="display:inline-block;width: 100px;" autocomplete="off" placeholder="开始日期"/>
	                        <s:textfield type="text" name="subcontract.customStrInfo.subcontEndDate" id="subcontEndDate" cssClass="form-control" style="display:inline-block;width: 100px;" autocomplete="off" placeholder="结束日期"/>
                        </div>
                    </td>
            ...
            <display:table style="text-align: left;"
                ...
                <display:column property="confirmTime" class="confirmTime" titleKey="pm.subcontract.confirmTime" format="{0,date,yyyy-MM-dd HH:mm:ss}"></display:column>
                <display:column property="paymentTime" class="paymentTime" titleKey="pm.subcontract.paymentTime" format="{0,date,yyyy-MM-dd HH:mm:ss}"></display:column>
                <display:column property="remark" class="text-input remark" titleKey="pm.subcontract.remark"></display:column>
                <display:column class="status" title="状态">${subcontractPaymentTable.customStrInfo.status}</display:column>
	            ...
	            <display:footer>
	                <%-- <s:if test="%{subcontractPaymentList.size() == 0}"> --%>
	                    <tr class="empty-template hidden inputLine">
	                        ...
	                        <td class="text-input amount"><input name="subcontractPaymentList[0].amount" class="form-control" readonly="readonly"></td>
	                        ...
	                        <td>
	                            <span class="glyphicon glyphicon-plus text-success" style="display:none;"></span>
        ...
        <script type="text/javascript">
	        $(function(){
                ...
                function initTableInput(allowInput) {
                	...
                	$("#subcontractPaymentTable tbody tr.readLine .glyphicon-minus").hide();
                	$("#subcontractPaymentTable tbody tr.inputLine .text-input").each(function(index) {
	        	...
	        	function checkPaymentForm() {
                   /*  var isEm = <s:property value='user.isHasRole(13)'/> || false;
                    ...
                    var selectors = "#subcontractPaymentForm input, .glyphicon-plus, .glyphicon-minus";
                    ...
                    $(".inputLine:not(.empty-template)").each(function() {
                    	...
                    	var amount = Number($.trim($(this).find(".amount input").val()).replace(",", "")) || 0;
                    	if (!ratio || !amount) {
                ...
                $(document).off("change", ".ratio input[name*='ratio']");
                ...
                $(document).on("change", "#subcontractPaymentTable tbody .amount");
