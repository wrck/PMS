...
//				targetName: "",
		...
		template: `<div v-if="isPermit">
						...
						<template v-else-if="field.type == 'button'">
							<button :id="field.cssId || field.field" :type="field.render(targetValue, field) || 'button'" class="btn" :class="getSelfClass(field) || field.cssClass" :data-btn-type="field.field" style="margin:0 0.15rem" :style="field.cssStyle" :disabled="field.disabled || fieldReadonly" :readonly="fieldReadonly">{{field.name}}</button>
						</template>
						<template v-else-if="field.type == 'buttons'">
							<div class="form-group" :class="getGroupClass(field) || groupClass">
