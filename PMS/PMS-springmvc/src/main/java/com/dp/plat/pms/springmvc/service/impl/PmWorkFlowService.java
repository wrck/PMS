...
public class PmWorkFlowService extends AbstractBaseService<PmWorkFlowMapper, PmWorkFlow> implements IPmWorkFlowService {
    ...
    @Override
    public List<PmWorkFlow> selectRunTasksByAssigneeAndProcessKeyAndTaskKey(
            PageParam<PmWorkFlow> pageParam, Integer assignee, String processKey, String taskKey
    ) {
        // TaskQuery taskQuery =
        ...
        TaskQuery taskQuery = taskService.createTaskQuery().processDefinitionKey(processKey)
                ...
                .taskVariableValueLessThanOrEqual("startTime", new Date()).active();
        ...
        for (Task task : taskList) {
            ...
            // Employee employee =
            // employeeDao.selectByPrimaryKey(Integer.valueOf(task.getAssignee()));
            // pmWorkFlow.setAssigneeName(employee != null ? (employee.getWorkNo() + "-" +
            // employee.getName()) : "");
            // } else {
            ...
            // Group group =
            // identityService.createGroupQuery().groupId(link.getGroupId()).singleResult();
            // pmWorkFlow.setAssigneeName(group != null ? group.getName() : "");
            // } else {
            // pmWorkFlow.setAssignee(link.getUserId());
            // Employee employee =
            // employeeDao.selectByPrimaryKey(Integer.valueOf(link.getUserId()));
            // pmWorkFlow.setAssigneeName(employee != null ? (employee.getWorkNo() + "-" +
            // employee.getName()) : "");
            // }
            // }
    ...
    @Override
    public List<PmWorkFlow> selectRunTasksByAssigneeAndProcessKeyAndTaskKey(
            PageParam<PmWorkFlow> pageParam, Integer assignee, List<String> processKeys, List<String> taskKeys
    ) {
        // TaskQuery taskQuery =
        // taskService.createTaskQuery().processDefinitionKeyIn(processKeys)
        // .taskCandidateOrAssigned(assignee.toString());
        // taskQuery = taskQuery.or();
        // for (String taskKey : taskKeys) {
        // taskQuery = taskQuery.taskDefinitionKey(taskKey);
        // }
        // taskQuery.endOr().taskVariableValueLessThanOrEqual("startTime", new
        // Date()).active();// startTime在planStepListener中赋值，取考核步骤的开始时间，默认值当前系统时间
        NativeTaskQuery nativeTaskQuery = taskService.createNativeTaskQuery().sql("");
        // NativeTaskQuery nativeTaskQuery =
        // taskService.createNativeTaskQuery().sql(PerfSqlConstant.WORKBENCH_TASK_SQL);
    ...
    public PmWorkFlow currentParticipantWorkFlow(PmWorkFlow pmWorkFlow, Object planParticipant) {
        ...
        if (!StringUtils.isNotEmpty(taskId)) {
            ...
            // runtimeService.createProcessInstanceQuery().processDefinitionId(task.getProcessDefinitionId()).processInstanceId(task.getProcessInstanceId()).active().singleResult();
            //
            ...
            // PlanObjectiveAppraiserRelationshipVO appraiser =
            // (PlanObjectiveAppraiserRelationshipVO) taskService
    ...
    private NativeHistoricTaskInstanceQuery candidateOrAssignedHistoricTaskQuery(Integer assignee, String fuzzy) {
        ...
        NativeHistoricTaskInstanceQuery taskQuery = // + "GROUP BY RES.`PROC_INST_ID_` ORDER BY RES.END_TIME_ DESC");
                historyService.createNativeHistoricTaskInstanceQuery()
                        .sql("SELECT * FROM ACT_HI_TASKINST RES INNER JOIN ("
    ...
    public String startProcess(PmWorkFlow pmWorkFlow, Object entity) {
        ...
        // runtimeService.setProcessInstanceName(processInstanceId,
    ...
    public PmWorkFlow decoratorEntity(PmWorkFlow pmWorkFlow) {
        ...
        temp.setStatus(PmWorkFlow.PENDING);
...
//		String objType = pmWorkFlow.getObjType();
        ...
        if (DataType.DISPATCH_SETTLEMENT.equals(dataType)) {
            DispatchSettlement settlement = null;
            if (entity instanceof DispatchSettlement) {
                settlement = (DispatchSettlement) entity;
            } else {
                settlement = JSON.toJavaObject((JSON) JSON.toJSON(entity), DispatchSettlement.class);
            }
            SettlementVO settlementVO = new SettlementVO();
            BeanUtils.copyProperties(settlement, settlementVO);
            IDispatchProjectService dispatchProjectService = SpringContext.getBean("dispatchProjectService",
                    IDispatchProjectService.class);
