...
(function (global, factory) {
      ...
      draggable: function draggable(modal, dragHandle, dragCancel) {
        ...
        var dragStart = function dragStart(e) {
          ...
          if (_this.options.multiInstances) {
        	  //update by j01441 2019-02-16  点击后z-index变化会导致预览框会在模态框下层
   //         modal.css('z-index', ++PUBLIC_VARS['zIndex']);
    ...
    function () {
      ...
      _proto.getImageScaleToStage = function getImageScaleToStage(stageWidth, stageHeight ,img) {
        var scale = 1;
        if(img == undefined){
        	img = this.img;
        }
        if (!this.isRotated) {
          scale = Math.min(stageWidth / img.width, stageHeight / img.height, 1);
        } else {
          scale = Math.min(stageWidth / img.height, stageHeight / img.width, 1);
        }
//          scale = Math.min(stageWidth / img.height, stageHeight / img.width, 1);
        return scale;
      };
      ...
      _proto.setImageSize = function setImageSize(img) {
        ...
        var scale = this.getImageScaleToStage(stageData.w, stageData.h ,img);
        this.$image.css({
      ...
      _proto.loadImg = function loadImg(imgSrc, fn, err) {
        ...
        preloadImg(imgSrc, function (img) {
          ...
          if(_this2.groupData[ _this2.groupIndex].orientation != undefined){
        	  _this2.rotate(-_this2.groupData[ _this2.groupIndex].orientation);
          }
          if (fn) {
            fn.call();
          }
