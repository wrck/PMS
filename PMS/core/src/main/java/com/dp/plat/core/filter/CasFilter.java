...
public class CasFilter extends org.apache.shiro.cas.CasFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
    	...
    	if (token.getCredentials() == null) {
    		AuthenticationException noTicket = new AuthenticationException("ignore no ticket token");
    		noTicket.setStackTrace(new StackTraceElement[0]);
    		return onLoginFailure(token, noTicket, request, response);
    	}
