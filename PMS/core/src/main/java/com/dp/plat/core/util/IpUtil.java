...
public class IpUtil {
	...
	public static void main(String[] args) {
		String ip = "10.102.0.106";// ip
		String mask = "26";// 位数，如果只知道子网掩码不知道位数的话在参考getMaskMap()方法
		...
		// 判断一个IP是否属于某个网段
		boolean flag = isInMarkRange("10.2.0.0", "10.3.0.0/17");
	 ...
	 */
	public static boolean isInMarkRange(String ip, String cidr) {
		String[] ips = ip.split("\\.");
	...
	/**
	 * 功能：判断一个IP是不是在指定的ip范围内：isInRange("192.168.9.3", "192.168.9.1-192.168.9.10");
	 */
	public static boolean isInRange(String ip, String ipRange) {
		if (ipRange == null || ip == null) {
			return false;
		}
		...
		Long startIp = getIpFromString(ips[0]);
		Long endIp = getIpFromString(ips[1]);
		Long ipLong = getIpFromString(ip);
		return (startIp <= ipLong && ipLong <= endIp) || (endIp <= ipLong && ipLong <= startIp);
	}
