...
<div class="modal-body">
	<form id="export-form-form" name="export-form" action="${pageContext.request.contextPath}/export/dataExport.xls" method="POST" class="form-horizontal form-inline">
		<input type="hidden" name="columns" value="">
		<input type="hidden" name="objectName" value="${objectName}">
		<input type="hidden" name="objectKV" value="${objectKV}">
		<input type="hidden" name="pageParamKV" value="${pageParamKV}">
		<input type="hidden" name="fullServiceName" value="${fullServiceName}">
...
<script>
	...
	$(function() {
		...
		$(document).on("click","#export-form-form button[type='button'][id*='select']",function(){
			var id =  $(this).attr("id");
			if(id == "selectOne") {
		...
		$(document).on("click","button[type='submit']",function(){
			...
			var columns = $("#exprotColumnsSelected").val();
			console.log(columns);
			if(columns) {
				$("#export-form-form input[name='columns']").val(columns.join(";"));
			}
			console.log($("#export-form-form input[name='columns']").val());
			var action = $("#export-form-form").attr("action");
			var path = action.split(".");
			var suffix = $("#export-form-form input[name='suffix']:checked").val();
			console.log(suffix);
			$("#export-form-form").attr("action", path[0] + suffix);
			console.log($("#export-form-form").attr("action"));
			$("#export-form-form").submit();
			$("#orderStatusTable_processing").hide();
			modals.closeWin("exportData");
