...
(function() {
    ...
    modals.openWin=function(config){ 
    	...
    	//attach shown event
    	$("#"+winId).off('shown.bs.modal', this.adjustWin);
...
//				}
				...
				if(contentHeight > scrollHeight){
					$(this).find(contentClass).slimScroll({destroy:true});
					$(this).find(contentClass).slimScroll({
...
*/         
    ...
    modals.hideWin=function(winId, hideCallback){
    	winId=winId||_win_config.winId;
    	$("#"+winId).modal('hide');
    }
    
    modals.showWin=function(winId, showCallback){
    	winId=winId||_win_config.winId;
    	var events = $._data($("#" + winId)[0], "events");
    	if (!(events && events['shown'])) {
    		// 判断是否有调整视窗的滚动条事件
    		$("#"+winId).on('shown.bs.modal', this.adjustWin);
    		/*$("#"+winId).one('shown.bs.modal', function(){
    			if($.fn.slimScroll){
    				var windowHeight = $(window).height();
    				var contentHeight = $(this).find(".modal-body").height();
    				var contentMarginTop = $(this).find(".modal-dialog").css("marginTop").replace("px",'');
    				var contentMarginBottom = $(this).find(".modal-dialog").css("marginBottom").replace("px",'');
    				var scrollHeight = windowHeight - contentMarginTop - contentMarginBottom;
    				if(contentHeight > scrollHeight){
    					$(this).find(".modal-body").slimScroll({destroy:true});
    					$(this).find(".modal-body").slimScroll({
    						height: windowHeight - 111,
    						size: 8,
    						disableFadeOut: true,
    						allowPageScroll: true,
    ...
    modals.adjustWin= function(winId) {
    	var _this = this;
    	if ($(this).is('.modal')) {
    		_this = this;
