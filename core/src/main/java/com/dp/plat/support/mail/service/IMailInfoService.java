 ...
 * 
 */
package com.dp.plat.support.mail.service;
...
public interface IMailInfoService {
	...
	/**
	 * 查找未发送邮件,预期发送时间小于当前时间, 发送失败次数小于指定次数
	 * 
	 * @param failedCount
	 * @return
	 */
	...
	int insert(MailInfo t);

	int insertSelective(MailInfo t);

	int updateByPrimaryKeySelective(MailInfo t);

	int updateByPrimaryKey(MailInfo t);
