...
public class RevokeTaskCmd implements Command<Integer> {
	...
	public RevokeTaskCmd(String historyTaskId, String processInstanceId, RuntimeService runtimeService,
			IWorkflowService workflowService, HistoryService historyService) {
	...
	public Integer execute(CommandContext commandContext) {
		...
		// 获得历史节点
		HistoricActivityInstanceEntity historicActivityInstanceEntity = getHistoricActivityInstanceEntity(
				historyTaskId);
		...
		if (processInstance != null) {
			...
			currentTasks = this.workflowService.getCurrentTaskInfo(processInstance);
			for (Task currentTask : currentTasks) {
				currentTaskId = currentTask.getId();
				HistoricTaskInstance hti = this.historyService.createHistoricTaskInstanceQuery().taskId(currentTaskId)
						.singleResult();
				// 下一结点已经通过,不能撤销。
		...
		for (Task currentTask : currentTasks) {
			...
			// Context.getCommandContext().getCommand().execute((CommandContext)
			// cmd);
	...
	public HistoricActivityInstanceEntity getHistoricActivityInstanceEntity(String historyTaskId) {
		...
		String historicActivityInstanceId = jdbcTemplate
				.queryForObject("select id_ from ACT_HI_ACTINST where task_id_=?", String.class, historyTaskId);
	...
	public void deleteActiveTasks(String processInstanceId) {
		Context.getCommandContext().getTaskEntityManager().deleteTasksByProcessInstanceId(processInstanceId, "revoke",
				true);
	...
	public void deleteHistoryActivities(String historyTaskId, String processInstanceId) {
		...
		List<HistoricActivityInstance> list = this.historyService.createHistoricActivityInstanceQuery().unfinished()
	...
	public ActivityImpl getActivity(HistoricActivityInstance historicActivityInstanceEntity) {
