 ...
 */
package com.dp.plat.activiti.converter;
		...
		if (objectNode.get(EDITOR_CHILD_SHAPES) != null) {
			for (JsonNode jsonChildNode : objectNode.get(EDITOR_CHILD_SHAPES)) {
				...
				if (STENCIL_SEQUENCE_FLOW.equals(stencilId) == false) {
					...
					// 开始结束标签
//					JsonNode properties = jsonChildNode.get(EDITOR_SHAPE_PROPERTIES);
//					if (properties == null || StringUtils.isBlank(properties.get(PROPERTY_NAME).asText())
//							|| !((stencilId.startsWith("Start") || stencilId.startsWith("End"))
//									&& stencilId.endsWith("Event"))) {
//						continue;
//					}
//					GraphicInfo graphicLabelInfo = new GraphicInfo();
//					graphicLabelInfo.setX(lowerRightNode.get(EDITOR_BOUNDS_X).asDouble() + upperLeftNode.get(EDITOR_BOUNDS_X).asDouble() / 2 + parentX);
//					graphicLabelInfo.setY(lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble() + parentY);
//					
//					graphicLabelInfo.setWidth(lowerRightNode.get(EDITOR_BOUNDS_X).asDouble() - graphicLabelInfo.getX() + parentX);
//					graphicLabelInfo.setHeight(lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble() - graphicLabelInfo.getY() + parentY);
//					
//					bpmnModel.addLabelGraphicInfo(BpmnJsonConverterUtil.getElementId(jsonChildNode), graphicLabelInfo);
				} else {
					JsonNode properties = jsonChildNode.get(EDITOR_SHAPE_PROPERTIES);
					if (properties == null || properties.get(PROPERTY_NAME) == null ||StringUtils.isBlank(properties.get(PROPERTY_NAME).asText())) {
						continue;
					}
					GraphicInfo graphicInfo = new GraphicInfo();
					...
					JsonNode dockers = jsonChildNode.get(EDITOR_DOCKERS);
					ObjectNode lowerRightNode = (ObjectNode) boundsNode.get(EDITOR_BOUNDS_LOWER_RIGHT);
					ObjectNode labelLeftNode = null;
					ObjectNode labelRightNode = null;
					ObjectNode firstDockerNode = lowerRightNode;
					boolean isDocker = false;
					if (dockers.size() > 2) {
						firstDockerNode = (ObjectNode) dockers.get(1);
						isDocker = true;
						// graphicInfo.setX(lowerRightNode.get(EDITOR_BOUNDS_X).asDouble()
						// + parentX);
						// graphicInfo.setY(lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble()
						// + parentY);
					}
					...
					// 判断是否在左下角三角形内
					double lowerLeftNodeX = upperLeftNode.get(EDITOR_BOUNDS_X).asDouble();
					double lowerLeftNodeY = lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble();

					double upperLeftNodeX = upperLeftNode.get(EDITOR_BOUNDS_X).asDouble();
					double upperLeftNodeY = upperLeftNode.get(EDITOR_BOUNDS_Y).asDouble();

					double lowerRightNodeX = lowerRightNode.get(EDITOR_BOUNDS_X).asDouble();
					double lowerRightNodeY = lowerRightNode.get(EDITOR_BOUNDS_Y).asDouble();

					double firstDockerNodeX = firstDockerNode.get(EDITOR_BOUNDS_X).asDouble();
					double firstDockerNodeY = firstDockerNode.get(EDITOR_BOUNDS_Y).asDouble();
					// 叉乘法
					double signOfTrig = (lowerLeftNodeX - upperLeftNodeX) * (lowerRightNodeY - upperLeftNodeY)
							- (lowerLeftNodeY - upperLeftNodeY) * (lowerRightNodeX - upperLeftNodeX);
					double signOfAB = (lowerLeftNodeX - upperLeftNodeX) * (firstDockerNodeY - upperLeftNodeY)
							- (lowerLeftNodeY - upperLeftNodeY) * (firstDockerNodeX - upperLeftNodeX);
					double signOfCA = (upperLeftNodeX - lowerRightNodeX) * (firstDockerNodeY - lowerRightNodeY)
							- (upperLeftNodeY - lowerRightNodeY) * (firstDockerNodeX - lowerRightNodeX);
					double signOfBC = (lowerRightNodeX - lowerLeftNodeX) * (firstDockerNodeY - lowerRightNodeY)
							- (lowerRightNodeY - lowerLeftNodeY) * (firstDockerNodeX - lowerRightNodeX);
					boolean d1 = (signOfAB * signOfTrig >= 0);
					boolean d2 = (signOfCA * signOfTrig >= 0);
					boolean d3 = (signOfBC * signOfTrig >= 0);

					if (dockers.size() >= 2) {
						labelLeftNode = (ObjectNode) dockers.get(0);
						labelRightNode = (ObjectNode) dockers.get(dockers.size() - 1);
					}
					int textLength = properties.get(PROPERTY_NAME).asText().length();
					double lw = 10 * textLength, lh = 14;
					if (labelLeftNode != null && labelRightNode != null
							&& (lowerRightNodeX - upperLeftNodeX) <= lw * 2) {
						lw = Math.abs(labelLeftNode.get(EDITOR_BOUNDS_X).asDouble()
								- labelRightNode.get(EDITOR_BOUNDS_X).asDouble()) * 1.5;
						lh = Math.abs(labelLeftNode.get(EDITOR_BOUNDS_Y).asDouble()
								- labelRightNode.get(EDITOR_BOUNDS_Y).asDouble()) * 2;
					}
					if (firstDockerNode.toString().equals(upperLeftNode.toString())) {
						lw = 0;
					} else if (firstDockerNode.toString().equals(lowerRightNode.toString()) && !isDocker) {
						lw *= 2;
					} else if (d1 && d2 && d3 && (lowerRightNodeX - firstDockerNodeX) > lw) {
						System.out.println(properties.get(PROPERTY_NAME).asText());
						lw = 0;
					}
					graphicInfo.setX(firstDockerNode.get(EDITOR_BOUNDS_X).asDouble() + parentX - lw);
					graphicInfo.setY(firstDockerNode.get(EDITOR_BOUNDS_Y).asDouble() + parentY - lh);

					// graphicInfo.setX(firstDockerNode.get(EDITOR_BOUNDS_X).asDouble()
					// + parentX);
					// graphicInfo.setY(firstDockerNode.get(EDITOR_BOUNDS_Y).asDouble()
					// + parentY);

					bpmnModel.addLabelGraphicInfo(BpmnJsonConverterUtil.getElementId(jsonChildNode), graphicInfo);
				}
			}
		}
	}
