...
/**
 * 重载CallActivityJsonConverter，增加CallActivity的inheritVariables属性
 * 
 * @author w02611
 */
public class CallActivityJsonConverter extends org.activiti.editor.language.json.converter.CallActivityJsonConverter {
	final String PROPERTY_CALLACTIVITY_INHERITVARIABLES = "callactivityinheritvariables";
...
//		// 是否继承父流程的变量
//		propertiesNode.put(PROPERTY_CALLACTIVITY_INHERITVARIABLES, callActivityisInheritVariables());
	...
	protected FlowElement convertJsonToElement(JsonNode elementNode, JsonNode modelNode,
		...
		if (StringUtils.isNotBlank(getPropertyValueAsString(PROPERTY_CALLACTIVITY_INHERITVARIABLES, elementNode))) {
			// 是否继承父流程的变量
			callActivity.setInheritVariables(
					Boolean.valueOf(getPropertyValueAsString(PROPERTY_CALLACTIVITY_INHERITVARIABLES, elementNode)));
		}
					...
					if ((sourceNode != null && StringUtils.isNotEmpty(sourceNode.asText()))
						...
						if (StringUtils.isNotEmpty(getValueAsString(PROPERTY_IOPARAMETER_SOURCE, itemNode))) {
							parameter.setSource(getValueAsString(PROPERTY_IOPARAMETER_SOURCE, itemNode));
								...
								.isNotEmpty(getValueAsString(PROPERTY_IOPARAMETER_SOURCE_EXPRESSION, itemNode))) {
